...
import { FaDatabase, FaFileContract, FaSearch, FaUsers } from "react-icons/fa";
...
const MOU = () => {
  ...
  const filterChange = (e) => {
    ...
    if (paramStr == "collaborators") {
      let upList = searchFrom.filter(
        (file) =>
          file[paramStr][0].includes(data) || file[paramStr][1].includes(data)
      );
      setFiles(upList);
    }
  ...
  const fetchMou = async () => {
    try {
      const response = await axios.get(
        "https://api.linkagesagreements.oauife.edu.ng/api/v1/memorandums?limit=10",
        {
          headers: {
    ...
    <div>
      <div className="lg:pl-14 lg:pt-3 bg-[#F1F1F1] w-[100vw] h-[92vh] no-scrollbar overflow-auto  lg:w-[80vw]">
       ...
       to-[#010080]"
            ...
            <div
              className="flex gap-2 lg:justify-between px-4 py-4 lg:pt-2   lg:pr-20 bg-gradient-to-r from-[#FECC48]
       to-[#010080] lg:bg-none"
            >
              <img src="iconstest.jpg" className="w-24 rounded-full" />
              <h3 className="lg:text-2xl lg:ml-6 font-bold mt-4 text-base text-white lg:mt-0 lg:text-[#211A79]">
            ...
            <div className="flex px-4 mt-4 lg:mt-0">
              ...
              <div
                className="bg-white p-4 ml-5 rounded-md hover:text-white
              ...
              <table className="w-11/12 mr-6 border-collapse border bg-white border-gray-200">
                <thead className="bg-black text-white">
                  <tr>
                    ...
                    <th className="text-left p-4 font-semibold border-b">
                      File
                    </th>
                    <th className="text-left p-4 font-semibold border-b">
                      Title
